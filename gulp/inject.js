...
gulp.task('inject', ['scripts', 'styles', 'injectAuth', 'inject404', 'copyVendorImages'], function () {
  ...
  var scriptPaths = [
    path.join(conf.paths.src, '/appSettings.js'), //added by Dave
    path.join(conf.paths.src, '/shared/**/*.js'), //added by dave
  ...
  if (isDev) {
    scriptPaths.splice(1, 0, path.join(conf.paths.src, '/appSettings.dev.js')); //added by Dave
  }
  var injectScripts = gulp.src(scriptPaths)
